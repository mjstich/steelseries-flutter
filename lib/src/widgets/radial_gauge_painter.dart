...
import 'package:steelseries_flutter/src/draw/radial.dart';
...
class RenderRadialGauge extends RenderBox {
  RenderRadialGauge({
    ...
    FontTypeEnum? fontType,
        ...
        _odometerParams = odometerParams,
        _useOdometer = useOdometer,
        _section = section,
        _area = area,
  ...
  OdometerParameters? get odometerParams => _odometerParams;
  OdometerParameters? _odometerParams;
  set setOdometerParams(OdometerParameters? odometerParams) {
    if (_odometerParams == odometerParams) return;
    _odometerParams = odometerParams;
    markNeedsPaint();
  }
  ...
  bool? _useOdometer;
  set setUseOdometer(bool? useOdometer) {
    if (_useOdometer == useOdometer) return;
    _useOdometer = useOdometer;
    markNeedsPaint();
  }
  ...
  bool hitTestSelf(Offset position) {
    ...
    if (radialPath.contains(calulatedPosition)) {
      return true;
    } else if (radialPath.contains(position)) {
      return true;
    } else {
      return false;
    }
    // Return false if the position is outside the radial gauge
  }
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    radialPath.addArc(rect, 0, TWO_PI);
    radialPath.close();
